...
class LeelaBoard:
    ...
    def __init__(self, chess_board = None, *args, **kwargs):
    ...
    def generate_legal_indices(self):
        '''Get legal move indices for the model output'''
        lms = list(self.generate_legal_moves())
        return self.lcz_uci_to_idx([m.uci() for m in lms]), lms
    ...
    def lcz_idx_to_uci(self, idx_list):
        # Return list of uci moves for this board position, given NN policy output indexes
        ...
        # find the uci move for each index
        return [uci_idx_dct.find(idx) for idx in idx_list]
